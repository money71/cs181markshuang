...
class NeuralNet(object):
    ...
    def check_layers(cls, cInputs, listLayer):
        ...
        if cInputs != listLayer[0].layer_input_size():
            return False
        for layerFst,layerSnd in izip(listLayer[:-1], listLayer[1:]):
...
def binary_encode_label(iLabel):
    ...
    >>> print " ".join("%.1f" % dbl for dbl in binary_encode_label(6))
    0.1 0.9 0.9 0.1"""
    ret = []
    i = iLabel
    for j in range(4):
...
def update_net(net, inst, dblLearningRate, listTargetOutputs):
    ...
    """
    listDblInput = inst.listDblFeatures
...
def main(argv):
    ...
    parser.add_option("--enable-stopping", action="store_true",
                      ...
                      help="detect when to stop training early")
